...
void CCharacterCore::Tick(bool UseInput)
{
	...
	if (m_HookPower)
	{
		if(m_HookDecaytick > m_HookpwrDuration)
			m_HookPower = false;
		else 
			m_HookDecaytick++;
	}
	...
	if (m_Suit)
	{
		if(m_SuitDecaytick > m_SuitDuration)
			m_Suit = false;
		else 
			m_SuitDecaytick++;
	}
	...
	if (m_ReverseGravity)
	{
		if(m_GravityDecaytick >  m_GravityDuration)
			m_ReverseGravity = false;
		else
			m_GravityDecaytick++;
	}
	...
	else if(m_HookState == HOOK_FLYING)
	{
		...
		if(Hit)
		{
			if(Hit&CCollision::COLFLAG_NOHOOK && !(m_HookPower))
				GoingToRetract = true;
			else
	...
	if(m_HookState == HOOK_GRABBED)
	{
		...
		if(m_HookedPlayer != -1 && m_pWorld->m_apCharacters[m_HookedPlayer])
			if(m_pWorld->m_apCharacters[m_HookedPlayer]->m_Suit)
			{
				m_HookedPlayer = -1;
				m_HookState = HOOK_RETRACTED;
				m_HookPos = m_Pos;
...
void CCharacterCore::EnableSpecial(int Type, int Duration)
{
	...
	{
		m_HookPower = true;
		m_HookpwrDuration = Duration;
	}
	else if (Type == 2)
	{
		m_Suit = true;
		m_SuitDuration = Duration;
	}
	else if (Type == 3)
	{
		m_ReverseGravity = true;
		m_GravityDuration = Duration;
	}
}
...
void CCharacterCore::Write(CNetObj_CharacterCore *pObjCore)
{
	...
	pObjCore->m_SpecialHook = m_HookPower;
	pObjCore->m_SpecialSuit = m_Suit;
	pObjCore->m_SpecialGravity = m_ReverseGravity;
}
...
void CCharacterCore::Read(const CNetObj_CharacterCore *pObjCore)
{
	...
	m_HookPower = pObjCore->m_SpecialHook;
	m_Suit = pObjCore->m_SpecialSuit;
	m_ReverseGravity = pObjCore->m_SpecialGravity;
}
