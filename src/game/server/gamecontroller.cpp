...
bool IGameController::OnEntity(int Index, vec2 Pos)
{
	...
	else if(Index == ENTITY_ARMOR_5)
	{
		...
		SubType = SPECIAL_YELLOWARMOR;
	}
	else if(Index == ENTITY_ARMOR_10)
	{
		Type = POWERUP_SPECIAL;
		SubType = SPECIAL_REDARMOR;
	}
	else if(Index == ENTITY_HEALTH_10)
	{
		Type = POWERUP_SPECIAL;
		SubType = SPECIAL_MEGAHEALTH;
	}
	...
	{
		Type = POWERUP_SPECIAL;
		SubType = SPECIAL_HOOKPWR;
	}
	else if(Index == ENTITY_POWERUP_SUIT && g_Config.m_SvPowerups)
	{
		Type = POWERUP_SPECIAL;
		SubType = SPECIAL_POWERSUIT;
	}
	else if(Index == ENTITY_POWERUP_REVERSEGRAVITY && g_Config.m_SvPowerups)
	{
		Type = POWERUP_SPECIAL;
		SubType = SPECIAL_REVERSEGRAVITY;
	}
	if(Type != -1)
	{
